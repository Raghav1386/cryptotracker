...
const Coinarea = () => {
  ...
  const { filteredcoins, currentCurrency, setCurrentCurrency } = useContext(CryptoContext);
...
<div className='space-y-3 relative z-10'>
  {filteredcoins && filteredcoins.length > 0 ? (
    filteredcoins.slice(0, 12).map((item) => {
      if (!item || !item.current_price) return null;

      return (
        <Link to={`/crypto/${item.id}`} key={item.id} className='block p-4 bg-gray-800/30 backdrop-blur-md hover:bg-gray-700/40 rounded-xl border border-emerald-500/10 hover:border-cyan-500/30 transition-all duration-300 group'>
          <div className='md:hidden space-y-3'>
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-3'>
                ...
                <img src={item.image} alt={item.name} className='w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-emerald-500/20 to-cyan-500/20 p-0.5' />
                <div>
            ...
            <div className='flex items-center justify-between pt-2 border-t border-emerald-500/10'>
              <div className={`flex items-center gap-1 text-sm sm:text-base ${item.price_change_percentage_24h > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
  ...
  <div className='text-right'>
    ...
    <p className='text-center text-gray-400'>Loading or no coins found.</p>
  )}
</div>
